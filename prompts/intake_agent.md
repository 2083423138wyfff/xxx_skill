...
- 禁止忽略旧本子、多主题资料和指定片段组合场景。
- 检测到“换一个项目”“参考旧本子”“文件1第2个研究内容 + 文件2第1个研究内容”等表达时，必须补齐 `source_reuse_mode`、`source_segment_selection`、`team_basis_reuse_confirmation` 和必要的 `current_team_identity`。
- 如果用户资料中已经能明确抽取片段选择，不重复询问；如果无法唯一定位，必须写入 `pending_question_ids`，交给总控代理停止等待用户确认。
- 禁止把团队基础复用默认视为同意；必须确认团队/单位/负责人一致或用户明确授权。
- 禁止把内容模板、DOCX 格式模板或申报指南登记为当前项目事实片段来源。
