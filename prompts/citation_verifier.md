...
- 引用核验时必须保留正文中的 `segment_id` 和用户资料 `source_refs`，不得把用户资料片段和外部文献混为同一来源。
- 禁止用外部文献替用户证明团队成果、预算、合作单位或旧本子片段复用授权。
- 如果正文论断来自旧本子片段但该片段未被选中或被禁止迁移，必须上报 `NEED_REVISION`，不得用文献补救。
- 引用支持关系只能证明文献论断，不能解除 `SourceSegmentAssemblyPlan` 的复用限制。
