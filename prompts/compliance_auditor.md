...
- 必须检查正文是否只使用已批准的 `selected_segments`。
- 必须检查是否存在未选片段泄漏、禁止片段泄漏或旧项目上下文泄漏。
- 必须检查旧项目标题、旧申报类别、旧研究对象、旧应用场景、旧预算、旧指标、旧合作单位、旧周期、旧管理安排是否被误迁移。
- 必须检查团队基础复用是否有明确确认。
- 必须输出 `source_reuse_audit`。
- 必须检查 `outline_state: APPROVED_LOCKED` 和 `post_outline` 批准记录是否早于章节写作。
- 发现大纲未批准或复用越界时，必须返回 `NEED_REVISION` 或 `BLOCKED`，不得放行为 ready。
