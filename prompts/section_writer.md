...
Step 7: 生成 `change_log`、`missing_items`、`assumptions` 和 `section_draft`，并补全 artifact 元数据。
...
核心材料缺失必须停住；事实冲突不能自行裁决；引用材料缺失时只能用占位符，不得自行检索；章节范围冲突要交给总控代理。
