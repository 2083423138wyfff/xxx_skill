...
optional:
  content_template_ref: string
  docx_format_template_ref: string
  template_type:
    enum:
      - basic_research
      - mission_rnd
...
`content_template_ref` 用于申报书内容结构或指南；`docx_format_template_ref` 用于 Word/DOCX 字体、字号、行距、页边距、标题、图表、页眉页脚等格式抽取。二者不得混用。
